...
sim_ctrl <- function(n_types = c(500,300), agent_types = c("H","P"), n_days = 100, 
                     ...
                     duration = 7, init_infect = 2, pct_vaccinated = c(.85, .5),
                     ve = 0.9, contact_probs = NULL){
  
  # if the contact matrix is missing build it
  if (is.null(contact_probs)) {contact_probs <- matrix(c(0.01,0.01,0.01,0.001),nrow = 2)}
  ...
  list(n_agents = n_agents,             # Number of agents (patients or HCWs) to simulate
       ...
       pct_vaccinated = pct_vaccinated, # percent of each agent type vaccinated
       ve = ve,                         # vaccine efficacy
...
build_patients <- function(sim_params = sim_ctrl()){
  ...
  # draw HCW (agent type 1) who are vaccinated
  hcw_vac <- rbinom(n = sim_params$n_type[1], 
                    size = 1, 
                    prob = sim_params$pct_vaccinated[1])
  
  # draw patients(agent type 2) who are vaccinated
  pat_vac <- rbinom(n = sim_params$n_type[2], 
                    size = 1, 
                    prob = sim_params$pct_vaccinated[2])
  ...
  tibble(agent = 1:sim_params$n_agents,
         ...
         vaccinated = c(hcw_vac,pat_vac),
         trans_ind = NA)
}

# test that a random number are vaccinated
build_patients() %>% 
  group_by(agent_type) %>% 
  summarise(mean(vaccinated))
...
make_interactions <- function(data, sim_params = sim_ctrl()){ 
  ...
  for (i in infectious_index){
    ...
    trans_prob <- data$susceptibility * data$transmisibility[i] * sim_params$cm[i,] * 
      (1- data$vaccinated*sim_params$ve)
...
res <- run_sim(sim_params = sim_ctrl(susceptibility = 0.15))
...
############################################
#### Visualize Results for different VE ####
############################################

# generate results for different levels of vaccine efficacy

res_ve_high <- run_sim_mc(trials = 500, cores = 5,
                          sim_params = sim_ctrl(ve = 0.9))



res_ve_low <- run_sim_mc(trials = 500, cores = 5,
                          sim_params = sim_ctrl(ve = 0.2))

# combine results

combined_results <- bind_rows(mutate(res_ve_high,
                 ve_level = "high"),
          mutate(res_ve_low,
                 ve_level = "low"))

# visualize the average trend
combined_results %>% 
  pivot_wider(names_from = state, 
              values_from = n,
              values_fill = 0) %>% 
  group_by(time,ve_level) %>% 
